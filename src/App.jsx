...
export default function App() {
  ...
  useEffect(() => {
    fetchScans();
    const timer = setInterval(fetchScans, 4000);
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <form className="scan-form" onSubmit={submit}>
          ...
          <button type="submit">Start staged scan</button>
        </form>

        {error && <div className="error-box">{error}</div>}
        <p className="side-note">
          Stages are dependency-ordered. Later phases remain skipped until their required inputs are produced.
        </p>
      </aside>
            ...
            {scans.length === 0 ? (
              ...
              scans.map((scan) => (
                <button
                  type="button"
                  key={scan.id}
                  className={`scan-item ${selectedId === scan.id ? 'active' : ''}`}
          ...
          <section className="details-panel">
            <div className="details-header">
              <div>
                ...
                <small>{selected.output_dir}</small>
              </div>
              <span className={`status status-${selected.status}`}>{selected.status}</span>
            </div>

            <div className="progress-label">
              <span>{completed} / {stages.length || 16} stages complete</span>
            ...
            {running && (
              <p className="running-note">Running: Phase {running.phase} — {running.name}</p>
            )}

            <div className="stage-list">
              {stages.map((stage) => (
                <div className="stage-row" key={stage.stage_key}>
                  <div className="stage-number">{String(stage.phase).padStart(2, '0')}</div>
                  <div className="stage-main">
                    <strong>{stage.name}</strong>
                    <small>{stage.priority} priority · {stage.message || 'Waiting'}</small>
                  </div>
            ...
            <div className="panel-block">
              <h4>Live log</h4>
              <pre className="log-output">{selected.log || 'No log output yet.'}</pre>
            </div>
          </section>
        )}
      </main>
