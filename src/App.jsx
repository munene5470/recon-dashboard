...
export default function App() {
  ...
  const fetchSelected = async (scanId) => {
    ...
    try {
      ...
      const files = artifactData.artifacts || [];
      setArtifacts(files);
      const statusFile = files.find((file) => file.path === 'subs/tool-status.ndjson');
      if (statusFile) {
        const statusResponse = await fetch(`/api/scans/${scanId}/artifacts/${statusFile.path}`);
        const statusText = await statusResponse.text();
        setToolStatuses(statusText.split(/\r?\n/).filter(Boolean).flatMap((line) => {
          try { return [JSON.parse(line)]; } catch { return []; }
        }));
      } else setToolStatuses([]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => fetchSelected(selectedId), 1000);
  ...
  const deleteScan = async () => {
    ...
    setSelected(null); setSelectedId(null); setArtifacts([]); setArtifact(''); setToolStatuses([]); await fetchScans();
  };

  const stages = selected?.stages || [];
  const processed = stages.filter((stage) => ['completed', 'skipped', 'failed'].includes(stage.status)).length;
  const completed = stages.filter((stage) => stage.status === 'completed').length;
  const running = stages.find((stage) => stage.status === 'running');
  const progress = stages.length ? Math.round((processed / stages.length) * 100) : 0;
  const toolsProcessed = toolStatuses.filter((tool) => ['completed', 'skipped', 'failed'].includes(tool.status)).length;
  const toolsTotal = toolStatuses.length;
  const toolsPercent = toolsTotal ? Math.round((toolsProcessed / toolsTotal) * 100) : 0;
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand-block"><div className="brand-mark">ER</div><div><h1>Elite Recon</h1><small>Staged pipeline</small></div></div>
      ...
      {error && <div className="error-box">{error}</div>}<p className="side-note">Only assess domains you are authorized to test. Results and progress refresh automatically.</p>
    </aside>
    <main className="main-panel"><header className="topbar"><div><p className="eyebrow">PIPELINE CONTROL</p><h2>Recon workspace</h2></div><button className="ghost-button" type="button" onClick={fetchScans}>Refresh</button></header>
      ...
      {selected && <section className="details-panel"><div className="details-header"><div><p className="eyebrow">ACTIVE SCAN</p><h3>{selected.target}</h3><small>{selected.output_dir}</small></div><div className="action-row"><span className={`status status-${selected.status}`}>{selected.status}</span><button className="danger-button" type="button" onClick={deleteScan}>Delete scan</button></div></div>
        <div className="progress-label"><span>{processed} / {stages.length || 16} stages processed · {completed} completed</span><strong>{progress}%</strong></div><div className="progress-track"><div style={{ width: `${progress}%` }} /></div>
        {running ? <div className="running-banner"><span className="pulse-dot" /> <span>Currently running:</span><strong>{running.message || running.name}</strong><small>Phase {String(running.phase).padStart(2, '0')} · {running.name}</small></div> : <p className="running-note">No stage is currently running.</p>}
        {toolsTotal > 0 && <div className="tool-progress"><div className="progress-label"><span>Phase 1 tools: {toolsProcessed} / {toolsTotal} processed</span><strong>{toolsPercent}%</strong></div><div className="progress-track small"><div style={{ width: `${toolsPercent}%` }} /></div><div className="tool-badges">{toolStatuses.map((tool) => <span className={`tool-badge tool-${tool.status}`} key={tool.tool}><b>{tool.tool}</b><small>{tool.status}{tool.count !== undefined ? ` · ${tool.count}` : ''}</small></span>)}</div></div>}
        ...
        <div className="panel-block"><div className="section-heading"><h4>Live log</h4><button className="ghost-button" type="button" onClick={() => setLogOpen(!logOpen)}>{logOpen ? 'Collapse' : 'Expand'}</button></div>{logOpen && <pre className="log-output">{selected.log || 'No log output yet.'}</pre>}</div>
      </section>}
    </main>
  </div>;
}
