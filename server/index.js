...
function updateScan(id, fields) {
  ...
  db.run(`UPDATE scans SET ${keys.map((key) => `${key} = ?`).join(', ')}, updated_at = ? WHERE id = ?`, values, (error) => {
...
app.get('/api/scans/:id/report', (req, res) => {
  db.get('SELECT output_dir, target FROM scans WHERE id = ?', [req.params.id], (error, scan) => {
    if (error) return res.status(500).json({ error: error.message });
    if (!scan) return res.status(404).json({ error: 'Scan not found' });
    const reportPath = path.join(ROOT_DIR, scan.output_dir, 'report', 'report.md');
    if (!fs.existsSync(reportPath)) return res.status(404).json({ error: 'Report not available yet' });
...
app.post('/api/scans', (req, res) => {
  ...
  db.run('INSERT INTO scans (target, status, output_dir, aggressive, scope_file, resume_dir, diff_dir, log, summary) VALUES (?, ?, ?, ?, ?, ?, ?, "", "{}")', [record.target, record.status, record.output_dir, record.aggressive, record.scope_file, record.resume_dir, record.diff_dir], function (error) {
    if (error) return res.status(500).json({ error: error.message });
    const scan = { id: this.lastID, ...record };
    updateScan(scan.id, { status: 'running', log: `Starting scan for ${scan.target}` });
    startScan(scan);
