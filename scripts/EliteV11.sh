...
if [[ -n "$SCOPE_FILE" && -f "$SCOPE_FILE" ]]; then
  log "Scope file in effect: $SCOPE_FILE"
fi

# Minimal subdomain generation used by the UI backend.
mkdir -p "$OUT/subs"
...
# Alive detection stub.
printf 'https://%s\n' "$TARGET" > "$OUT/alive/alive_urls.txt"

# JS fix: count increment and regex for .js files with query/hash suffix.
mkdir -p "$OUT/js"
...
# Basic structured findings with OWASP-style categories.
