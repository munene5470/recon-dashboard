...
if command -v subfinder >/dev/null 2>&1; then
  run_subdomain_tool subfinder subfinder -silent -d "$TARGET"
else
  record_tool subfinder skipped 0
fi
if command -v assetfinder >/dev/null 2>&1; then
  run_subdomain_tool assetfinder assetfinder --subs-only "$TARGET"
else
  record_tool assetfinder skipped 0
fi

{ cat "$OUT/subs/raw/"*.txt 2>/dev/null || true; printf '%s\n' "$TARGET"; } \
  | normalize_hostnames | sort -u > "$OUT/subs/all.txt"
...
run_stage 02-dns "Resolving discovered hosts" '
  if command -v dnsx >/dev/null 2>&1; then
    dnsx -silent -l "$OUT/subs/all.txt" -o "$OUT/dns/resolved.txt"
  else
...
run_stage 03-alive "Discovering live HTTP services" '
  if command -v httpx >/dev/null 2>&1; then
    httpx -silent -l "$OUT/dns/resolved.txt" -o "$OUT/alive/alive.txt"
  else
...
run_stage 04-ports "Enumerating open ports" '
  if command -v naabu >/dev/null 2>&1; then
    naabu -silent -host-file "$OUT/dns/resolved.txt" -p 80,443,8080,8443 -o "$OUT/ports/open-ports.txt"
  else
    : > "$OUT/ports/open-ports.txt"
  fi
  printf "{\"open_ports\":%s}\n" "$(count_lines "$OUT/ports/open-ports.txt")" > "$OUT/ports/summary.json"
'
...
run_stage 05-crawl "Crawling live HTTP services" '
  ...
  if command -v katana >/dev/null 2>&1; then
    while IFS= read -r url; do
      [[ -n "$url" ]] || continue
      katana -silent -u "$url" -d 2 -jc -o "$OUT/crawl/$(printf "%s" "$url" | md5sum | cut -d" " -f1).txt" || true
...
run_stage 07-tech "Extracting JavaScript URLs" '
  grep -Eo "https?://[^\"[:space:]]+\.js([?#[^\"[:space:]]*)?" "$OUT/crawl/all_urls.txt" "$OUT/alive/alive.txt" 2>/dev/null \
    | sed "s/^[^:]*://" | sed "s/[),;>]$//" | sort -u > "$OUT/js/js_urls.txt" || true
  while IFS= read -r js_url; do
    [[ -n "$js_url" ]] || continue
    hash=$(printf "%s" "$js_url" | md5sum | cut -d" " -f1)
    curl -fsSL --retry 1 --connect-timeout 8 --max-time 30 "$js_url" -o "$OUT/js/downloads/$hash.js" || true
  done < "$OUT/js/js_urls.txt"
  if find "$OUT/js/downloads" -type f -name "*.js" -print -quit | grep -q .; then
    zip -qr "$OUT/js/js-files.zip" "$OUT/js/downloads"
  fi
...
for key in 10-misconfig 11-injection 12-access-control 13-takeover 14-secrets 15-nuclei; do
  stage "$key" skipped "Not enabled in safe discovery mode"
done

cat > "$OUT/findings/findings.json" <<EOF
{"target":"$TARGET","generated":"$(date -Iseconds)","pipeline_status":"completed","summary":{"subdomains":$TOTAL_SUBS,"resolved_hosts":$(count_lines "$OUT/dns/resolved.txt"),"alive_urls":$(count_lines "$OUT/alive/alive.txt"),"crawl_urls":$(count_lines "$OUT/crawl/all_urls.txt"),"js_urls":$(count_lines "$OUT/js/js_urls.txt"),"api_endpoints":$(count_lines "$OUT/api/api_endpoints.txt"),"graphql_endpoints":$(count_lines "$OUT/graphql/graphql_endpoints.txt")}}
EOF
